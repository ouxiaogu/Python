...
filename = "Result"
postfix = ".csv"
gauge_result = os.path.join(workpath,  filename+postfix)
...
def drawQuadState(df, filter_name, **args):
    ...
    if(args.has_key("save") & args["save"] == True):
        ...
        fig.savefig(os.path.join(directory, "{}_error_analysis.png".format(filter_name)), dpi=2*fig.dpi, frameon = False )
    else:
        plt.show()
    plt.close(fig)
...
#        break
    ...
    dict_outliers[group], dict_stat_by_group[group] = drawQuadState(df_slice, group, save = True)
...
out_name = filename + "_stat_by_group.txt"
