"""
-*- coding: utf-8 -*-
Created: peyang, 2018-01-25 00:07:20
...
def setAxisLim(ax, scale=False, scale_ratio=0.1):
    (xmin, xmax) =  ax.get_xlim()
    (ymin, ymax) =  ax.get_ylim()
    x_range = xmax-xmin
    y_range = ymax-ymin
    sratio = scale_ratio if scale == True else 0
    ax.set_xlim([xmin-x_range*sratio, xmax+x_range*sratio])
    ax.set_ylim([ymin-y_range*sratio, ymax+y_range*sratio])
...
def addLegend(axes):
    ...
    axes[0].legend(legend_handles, legend_labels, fontsize=10, framealpha = 0.5, loc='best') #   bbox_to_anchor=(-0.1, 1),
